...
namespace zoom
{
    ...
    class offline_analyzer : public analyzer
    {
        ...
        // new
        std::set<std::uint32_t> uniqueRtpTimestamps;
        // std::set<std::uint32_t> first100rtp;
        // std::set<std::uint32_t> first100ts;
    ...
    private:
        ...
        void _write_frame_log(const stream_analyzer &a, const struct stream_analyzer::frame &frame, double times, double rtps, int groupNumber, double diff);
